...
while one_or_two != "2":
    ...
    if one_or_two =="1":
        units - float(input("กรอกจำนวนไฟฟ้า:"))
        total_cost = calculate_electricity_cost(units)
        if units <=50:
            print ("ค่าบริการ: 25บาท")
            print (f"รวมค่าไฟทั้งสิ้น {units} หน่วย เป็นจำนวณเงิน: {total_cost:.2f} บาท")
        elif units <=100:
            print (f"1-50 หน่วย: 125บาท")
            print (f"50-{units} หน่วย: {(units-50)*3:.2f}บาท")
            print ("ค่าบริการ: 25บาท")
            print (f"รวมค่าไฟทั้งสิ้น {units} หน่วย  เป็นจำนวนเงิน: {total_cost:.2f}บาท")
        elif units <=200:
            print (f"1-50 หน่วย: 125บาท")
            print (f"51-100 หน่วย: 150บาท")
